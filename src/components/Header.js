...
import { deleteBoards, setBoardActive } from "../redux/BackendActions";
    ...
    <div className="p-4 fixed left-0 bg-white dark:bg-[#2b2c37] z-50 right-0">
      <header className="flex justify-between dark:text-white items-center">
        ...
        <div className="flex space-x-4 items-center md:space-x-6">
          <button className="button hidden md:block" onClick={() => setIsTaskModalOpen((prevState) => !prevState)}>+ Add New Task</button>
          <button onClick={() => setIsTaskModalOpen((prevState) => !prevState)} className="button py-1 px-3 md:hidden">+</button>
          <img
            onClick={() => {
              setBoardType("edit");
